	...
	steps {
		...
		bashNodeScript {
			...
			scriptContent = """
				...
				# work. It likely fails because some packages depend on the types
				# generated by the builds of other packages. When both builds run
				# in parallel and when the clean task deletes some types, many
				# builds can fail. Since we already have tsc running in other builds,
				# we can avoid wasting build time by just testing a package which
				# uses the copy-asset and transpile scripts.
